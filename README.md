...
`.babysit/TASK.md`, starts the detached supervisor, and hands back the commands
to watch it — instead of trying to do an hours-long task in one context window.
...
79 checks, all with stub worker sessions — no agent CLI, no model, no network.
...
supervisor, the prompt contract, and the state commands.
